...
#Creating the project folder for inFamix
#cd $homeFolder
#mkdir $name-project
#cp -r $gitRepos/$name/src $name-project/
#cp -r $gitRepos/$name/lib $name-project/

#Create .mse file with inFamix
#maybe check if it already exists
tVerveineJ="$(date +%s)"

./verveinej/verveine.extractor.java/verveinej.sh -Xmx2000m gitRepos/$name/src
